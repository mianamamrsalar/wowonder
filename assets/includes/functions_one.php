...
function Wo_UpdatePost($data = array()) {
    ...
    $post_title =Wo_Secure($data['title']);
    $post_desc = Wo_Secure($data['desc']);
    $post_tags = Wo_Secure($data['tags']);
    $user_id   = Wo_Secure($wo['user']['user_id']);
    $post_id   = Wo_Secure($data['post_id']);
    if (Wo_IsPostOnwer($post_id, $user_id) === false) {
        return false;
    ...
    if (!empty($post_title)) {
        if ($wo['config']['maxCharacters'] > 0) {
            if (strlen($post_title) > $wo['config']['maxCharacters']) {
            }
        }
        $link_regex = '/(http\:\/\/|https\:\/\/|www\.)([^\ ]+)/i';
        $i          = 0;
        preg_match_all($link_regex, $post_title, $matches);
        foreach ($matches[0] as $match) {
            $match_url = strip_tags($match);
            $syntax    = '[a]' . urlencode($match_url) . '[/a]';
            $post_title = str_replace($match, $syntax, $post_title);
        }
        $mention_regex = '/@([A-Za-z0-9_]+)/i';
        preg_match_all($mention_regex, $post_title, $matches);
        foreach ($matches[1] as $match) {
            $match         = Wo_Secure($match);
            $match_user    = Wo_UserData(Wo_UserIdFromUsername($match));
            $match_search  = '@' . $match;
            $match_replace = '@[' . $match_user['user_id'] . ']';
            if (isset($match_user['user_id'])) {
                $post_title  = str_replace($match_search, $match_replace, $post_title);
                $mentions[] = $match_user['user_id'];
            }
        }
    }
    ...
    $query_one = mysqli_query($sqlConnect, "UPDATE " . T_POSTS . " SET `postText` = '{$post_text}', `postFileName` = '{$post_title}', `postDescription` = '{$post_desc}', `postTags` = '{$post_tags}' WHERE `id` = {$post_id}");
    if ($query_one) {
        if ($wo['config']['cacheSystem'] == 1) {
            $cache->delete(md5($data['post_id']) . '_P_Data.tmp');
        }
