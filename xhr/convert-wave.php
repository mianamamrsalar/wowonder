...
$audio_id = '44';//secure($_POST['id']);
$getIDAudio = $db->where('post_id', $audio_id)->getValue(T_POSTS, 'id');
...
if ($put1 && $put2 && file_exists($darkImageFinal) && file_exists($lightImageFinal)) {
	...
	$update = $db->where('post_id', $audio_id)->update(T_POSTS, ['dark_wave'=> $darkImageFinal, 'light_wave'=> $lightImageFinal]);
	if ($update) {
		$data['status'] = 200;
	}
}
